...
export const getAvailableProducts: RequestHandler = async (req, res, next) => {
  try {
    const { start_date, end_date, term } = req.query;

    // Validar y convertir a string
    const startDateStr = typeof start_date === 'string' ? start_date : '';
    const endDateStr = typeof end_date === 'string' ? end_date : '';

    // Validar que no estén vacíos
    if (!startDateStr || !endDateStr) {
      res.status(400).json({
        message: 'Las fechas de inicio y fin son requeridas como cadenas.',
      });
      return;
    }

    // Ahora puedes usar moment con seguridad
    const startDate = moment(startDateStr, 'YYYY-MM-DD', true).toDate();
    const endDate = moment(endDateStr, 'YYYY-MM-DD', true).toDate();
    ...
    // Consulta principal con cálculo de disponibilidad
    const products = await Product.findAll({
          ...
          include: [
            {
              ...
              where: {
                ...
                status: {
                  [Op.in]: ['pending_return', 'completed', 'with_issues']
                }
              }
            }
          ]
      ...
      group: ['Product.id'], // Necesario por el SUM
      order: [[col('name'), 'ASC']],
      subQuery: false // Evita errores con GROUP BY en includes
    });
